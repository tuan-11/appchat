...
export const AuthContextProvider = ({children})=>{
      ...
      if (typeof users === 'object' && !Array.isArray(users)) {
        try {
          ...
          const userToUpdate = userObjects.find(user1 => {
            ...
            if (userToUpdate && docSnap.exists()) {
              const userData = docSnap.data();
              await client.updateUser({
              id: userToUpdate.id,
              name: newName, 
              image: userData.userUrl
            });
          } else {
    ...
    const updateStream = async (userId, token)=> {
        const chatClient = new StreamChat.getInstance('bt2cwnkjayw3', '8u648gm3myz87c2gfrjddbubahyqyydapx6sskcbyqbgagq3bgdyngyu7pw2nk6n');
        const client = await chatClient.connectUser({id: userId}, token);
        console.log('Connected user:', client);
        const users = await chatClient.queryUsers({}, { id: 1 });
        console.log('userTuan:', users);
        if (typeof users === 'object' && !Array.isArray(users)) {
          try {
            let name;
            let newImageUrl;
